...
m.readshapefile(r'd:\LISICKY\PhD\Python\Basemap\Areas\CR\okresy', 'areas')



# přidání informace do dané oblasti
df_poly = pd.DataFrame({'shapes': [Polygon(np.array(shape), True) for shape in m.areas], 
                        'Okres': [area['NAZEV_LAU1'] for area in m.areas_info]})
# =================================== načtení vstupu uživatele ====================================
df_input = pd.read_csv("navstevy.txt", skiprows = None)
counts = pd.DataFrame(df_input["Occurence"].value_counts())
counts["Okres"] = counts.index
...
df_poly = df_poly.merge(counts, on='Okres', how='left')
