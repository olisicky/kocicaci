...
def open(name):
    ...
    return df.iloc[:255,:]  # omezení pro oblast, která se dá číst.. pak mám velké flow!!!
def AD_flow(AD_values):
    ...
    # return 0.0005 * AD_values - 0.3749
def hysteresis(data):
    ...
    ref1 = data.iloc[115,1] # referenční bod z výstupních DAT! začátek hystereze
    ref2 = data.iloc[175,1]
    ref3 = data.iloc[235,1]
    hyst1 = data.iloc[116:125,:]
    hyst2 = data.iloc[176:185,:]
    hyst3 = data.iloc[236:245,:]
...
def mean_response():
    ...
    return export.iloc[0:170,:] # tohle záleží na počtu hodnot, které chci vykreslit.. omezeno tím, které indexy posílám do háje!
...
def opening(data, limits, hysteresis1, hysteresis2, hysteresis3):
    ...
    ax.text(350, 1.2, "Erwin range", fontsize=14, color = "blue")
    ax.text(330,3, "HYsteresis: {}".format(hysteresis1))
    ax.text(360, 4, "HYsteresis: {}".format(hysteresis2))
    ax.text(400, 5.5, "HYsteresis: {}".format(hysteresis3))
    # ax.axvspan(150, 360, alpha=0.1, color='red')   # ukázání oblasti, kde je pracovní oblast
    ax.axvspan(290, 480, alpha=0.2, color="blue")   # oblast AOS - Erwin
...
def increase(data):
    ...
    ax.text(350, 2.1, "Erwin range", fontsize=14, color = "blue")
    ax.axvspan(290, 480, alpha=0.2, color="blue")   # oblast AOS - Erwin
    ax.hlines(xmin = 100, xmax = 480, y = 1.5, linestyle= ":", color = "red", label = "1.5 % increase limit")
    ax.hlines(xmin = 100, xmax = 480, y = 1, linestyle= ":", color = "green", label = "1 % increase limit")
    ax.hlines(xmin=100, xmax=480, y=0, color="red", linewidth = 2, label="0 % increase limit")
    initial = np.linspace(0, 14, 15)
    hyst1 = np.linspace(115, 136, 22)
    hyst2 = np.linspace(175, 196, 22)
    hyst3 = np.linspace(235, 254, 20)
...
def comparison(limits):
    ...
    for i in names:
        ...
        df.columns = ["position", "AD"]
        df = df.iloc[:255,:]    # záleží na počtu dat!
        index = measure_groups.index[measure_groups['throttle'] == i]   # hledám index z předpřipraveného df, podle kterého budu přiřazovat barvu
        ...
        lines.append(i)
    # ========================================= CHECK BUTTONS =========================================================
    plt.subplots_adjust(left=0.2)   # posunutí kvůli těm check button
    # vytvoření možnosti CHECK BUTTON, abych mohl v grafu potom zaklikávat, o chci zobrazit
    rax = plt.axes([0.0, 0.1, 0.15, 0.9])
...
#         except:
#             pass
#     measure_groups = pd.DataFrame(files, columns=['throttle'], index = dates)
#     measure_groups = measure_groups.sort_index()    # sort podle data vytvoření!
#
#     unique = measure_groups.index.unique()  # nalezení unikátních indexů pro vytvoření skupin
#     groups = []
#     for item in unique: # cyklus, kde se mi to rozdělí podle jednotlivých dnů
#         item = measure_groups.loc[item]
#         groups.append(item)
#     return groups
# separe_days = grouping()
# print(separe_days)
comparison(limits)
